...
我们知道http请求是走filter，到达Servlet之后才进行拦截器处理。所以如果把跨域配置放在filter中就可以生效。但是具体的细节还是要注意，前端要设置http请求带上cookie。即需要将 `withCredentials`设置为**true，**表示请求时会附带cookie，我使用的是axios，所以可以这样设置：代码如下：
